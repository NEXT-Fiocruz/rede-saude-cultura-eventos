...
<div class="block" id="sidebarUser">
	...
	{if $isUserLoggedIn}
		...
		<ul>
			...
			<li><a class="perfil-sair" href="{url page="login" op="signOut"}">{translate key="plugins.block.user.logout"}</a></li>
			{if $userSession->getSessionVar('signedInAs')}
				<li><a class="perfil-sair" href="{url page="login" op="signOutAsUser"}">{translate key="plugins.block.user.signOutAsUser"}</a></li>
			{/if}
		</ul>
	{else}
